-- Agregar columna promo_image_url para el afiche / flyer del modal
ALTER TABLE public.ad_banners 
  ADD COLUMN IF NOT EXISTS promo_image_url TEXT;

-- Eliminar versión previa para permitir cambiar el tipo de retorno (RETURNS TABLE)
DROP FUNCTION IF EXISTS public.get_banners_for_user(UUID[], TEXT, INT);

-- Actualizar la función RPC get_banners_for_user para incluir promo_image_url
