-- Migración para sincronización de garantías comerciales desde Odoo hacia Supabase
-- Actualiza la función pública sync_odoo_supplier_data
    ...
    IF p_products IS NOT NULL AND jsonb_array_length(p_products) > 0 THEN
        FOR v_product IN SELECT * FROM jsonb_array_elements(p_products) LOOP
            IF v_product->>'sku' IS NOT NULL AND TRIM(v_product->>'sku') != '' THEN
                ...
                -- 2.1 AUTO-HOMOLOGACIÓN DE MARCA
                ...
                -- INSERT / UPSERT DEL PRODUCTO CON GARANTÍA
